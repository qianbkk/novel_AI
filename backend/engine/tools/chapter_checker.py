...
Migrated from novel_AI/tools/chapter_checker.py. Two-phase check:
1. Local regex-based (no LLM): point logic + level-up logic
2. LLM-based deep consistency (every 10 chapters)
"""
...
from ..config.paths import CHAPTERS_DIR_STR, OUTPUT_DIR_STR, SETTING_PATH_STR
...
def check_level_up_logic(chapter_num: int, text: str, current_level: str) -> list:
    ...
    for level_name, (num, _threshold) in POWER_LEVELS.items():
...
def scan_all_chapters(novel_id: str = "renqingzhai_v1") -> dict:
    ...
    for fname in chapter_files:
        ...
        with open(os.path.join(CHAPTERS_DIR_STR, fname), encoding="utf-8") as f:
            text = f.read()
        local_issues = check_point_logic(ch_num, text, prev_points) + \
                       check_level_up_logic(ch_num, text, current_level)
        for issue in local_issues:
            all_issues.append({"chapter": ch_num, "source": "local",
        ...
        if ch_num % 10 == 0:
            known_facts = {
                "characters": l2.get("hot", {}).get("character_states", {}),
                "protagonist_level": l2.get("hot", {}).get("protagonist_level", "感债者"),
                "protagonist_points": prev_points,
                "inventory": l2.get("hot", {}).get("inventory", []),
                "established_facts": [
                    f["fact"] for f in
                    l2.get("constraints", {}).get("established_facts", [])
                ],
            }
