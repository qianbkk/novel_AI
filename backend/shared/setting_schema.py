"""shared.setting_schema — 跨 app ↔ engine 的 JSON Schema 校验工具。

2026-07-25 抽离（修 P0 短板双向 import）。

之前 engine/agents/planner.py `from app.schema_validator import ...` 跨过包边界
（engine→app 反向 import）。把 schema_validator 移到 shared 后，engine 和 app
都从 shared import，方向单向。

为什么是纯函数模块：原 app.schema_validator 只 import jsonschema + .logging_setup
（后者仅用于 logger 命名），无 app 内部状态依赖。改用 stdlib logging 即可
独立运行。

兼容：app.schema_validator 仍然 re-export 所有函数，老代码不破。
"""
...
log = logging.getLogger("novel_ai.shared.setting_schema")
