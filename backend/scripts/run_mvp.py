...
用法:
    cd backend
    python -m scripts.run_mvp <project_id> [--api http://localhost:8000] [--chapters 1] [--select A]
...
def main() -> int:
    ...
    parser.add_argument("--api", default="http://localhost:8000", help="后端 base URL")
    ...
    with httpx.Client(base_url=args.api, timeout=600.0) as client:
        ...
        try:
            ...
            print(f"✗ 后端 {args.api} 不可达：{e}\n  请先在另一个终端跑: cd backend && uvicorn app.main:app --reload --port 8000")
            return 1
