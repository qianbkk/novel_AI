...
class TestSchemaValidatorFailFast:
    """JSON Schema 实现迁入 shared 后仍必须在导入期 fail-fast。

    `app.schema_validator` 只是兼容转发层；真正的依赖与 `_check` 实现在
    `shared.setting_schema`。测试应锁定职责所有者，而不是要求 shim 复制实现。
    """

    def test_jsonschema_imported_at_module_level(self):
        """jsonschema 必须存在于真实实现模块 namespace。"""
        import shared.setting_schema as mod
        assert "jsonschema" in mod.__dict__, (
            "jsonschema 必须从 shared.setting_schema 顶层导入，缺依赖时应 fail-fast"
        )

    def test_check_has_no_import_error_fallback(self):
        """真实 `_check()` 不能捕获 ImportError 后静默跳过校验。"""
        import inspect
        from shared.setting_schema import _check
        src = inspect.getsource(_check)
        assert "except ImportError" not in src, (
            "_check() 不应有 except ImportError，否则会静默跳过 schema 校验"
        )

    def test_module_top_level_imports_jsonschema(self):
        """真实实现模块源码顶层必须导入 jsonschema。"""
        import inspect
        from shared import setting_schema as mod
        ...
        assert any("jsonschema" in line for line in top_imports), (
            "shared.setting_schema 顶层必须 import jsonschema，"
            f"当前顶层 import: {top_imports}"
        )
