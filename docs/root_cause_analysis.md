...
### 日常开发
```bash
# 1. 改完代码后跑这个，warn=0 error=0 才算改完（PASS 数随 schema 演进）
python -m scripts.audit_project

# 2. 跑单元测试锁住不变量
python -m pytest tests/test_invariants.py -v
# 截至 2026-07-03：118/118 PASS
```
...
- 任何 PR 必须 audit warn=0 error=0 + 全部 invariant tests PASS（2026-07-03 共 118 个）
...
- `backend/tests/test_invariants.py` — 锁死测试（截至 2026-07-03 共 118 个）
