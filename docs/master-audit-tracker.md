...
91a2758  fix(memory+orchestrator): expire_constraints None 兜底 + recursion_limit 1500
fdfe414  feat(audit-P1-P4+P6): escalation memory gap + reconcile + adaptive audit
b4f9926  fix(audit-tracker-round4): G-1/2/3 + F-1/3/4/6/7
...
Round 4 完成后补:
```
b4f9926  fix(audit-tracker-round4): G-1/2/3 + F-1/3/4（实测报告 + round 3 follow-up）
fdfe414  feat(audit-P1-P4+P6): escalation memory gap + reconcile + adaptive audit
          (审计 P1 / P2 / P3 / P4 + 30 章实验发现的 P6 tracker parse 兜底)
91a2758  fix(memory+orchestrator): expire_constraints None 兜底 + recursion_limit 1500
          (300 章 v2 实测发现的 P7-fix；commit 后又 bump 到 2500 让 v3 跑完)
```
...
- **LLM JSON shape 漂移**：~96% 章节 tracker JSON 解析失败（LLM 返 null/str
  而非 int）— P6 修复让其不 crash，只 log warning
- **dedup 修法对未覆盖的 None 漂移**：P7-fix 让 `expire_constraints` 不崩
- **最终预算消耗**：~$15（300 ch × ~$0.05）
- **总耗时**：~6h（~1.4 min / 章，受 LLM 响应延迟影响）

**内容自检**：sample 阅读 ch_0001 / ch_0050 / ch_0085 / ch_0208 / ch_0276
（不同 arc 段位）—— 一致性保持（人物、地点、案件线连贯）。

**LLM concept 漂移**：用户给的 concept 是"陈青云修真"，但 LLM 自动
选择了"陆承都市情感悬疑"题材，原因是没有走 outline agent 让 concept
真正进入 chapter_goal。这是 driver 设计的局限——`gen_chapters_direct`
跳过 planner/outline 直接喂 placeholder_task，placeholder_task 的
chapter_goal 是 "第N章：推进剧情"（dummy），LLM 只能根据这个写。
若要让 LLM 真按 concept 写，先调 outline agent 生成真正带 concept 的
chapter_goal 再喂 task queue。